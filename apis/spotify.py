...
def get_formatted_tracklist_table_html(tracks: list):
    ...
    keys = ['name', 'album_image_url_small',
            'artist_name', 'album_name', 'share_url']
    df = df[keys]
    ...
    playlist_table = df.to_html(
        formatters=formatters, escape=False, index=False)
    playlist_table = playlist_table.replace('style="text-align: right;"', '')
    playlist_table = playlist_table.replace(
        '<tr>', '<tr style="border: solid 1px #CCC;">')
    playlist_table = playlist_table.replace(
